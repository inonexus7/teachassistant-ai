...
const TestCreator: FC<ChatbotProps> = ({ clearAnswer, setAnswer }) => {
    const [data, setData] = useState<any>({ lang: 'en', grade: 1, numMulti: 1, numShort: 1, numEssay: 1 })
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            const response: any = await fetch(`${serverUrl}/chatbot/test/creator`, {
                method: 'POST',
                headers: {
    ...
    return (<Grid item sm={12} md={5} lg={4} style={{ background: '#fff', padding: 30 }}>
        ...
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Grade Level" variant='standard' name="grade" type="number" onChange={handleChange} defaultValue={1} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Subject" variant='standard' name="subject" onChange={handleChange} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Test Name" variant='standard' name="test_name" onChange={handleChange} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Test Instructions (Any specific instructions or guidelines for test takers)" variant='standard' name="testInstruction" onChange={handleChange} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Key Testing goal/s (specify in what your are testing)" variant='standard' name="goal" onChange={handleChange} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Number of multiple-choice questions" variant='standard' name="numMulti" type="number" onChange={handleChange} defaultValue={1} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Number of short answer questions" variant='standard' name="numShort" type="number" onChange={handleChange} defaultValue={1} />
        </Box>
        <Box sx={{ marginY: 2 }}>
            <TextField fullWidth label="Number of essay questions" variant='standard' name="numEssay" type="number" onChange={handleChange} defaultValue={1} />
        </Box>
        <Box sx={{ marginY: 2 }}>
